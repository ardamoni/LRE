...
//If you have received a submission.
	...
	$allowedmimes = array ("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet","text/csv","application/vnd.ms-excel");
      ...
      if ($goodtogo){
		try {
		if (!move_uploaded_file ($_FILES['uploadedfile']['tmp_name'],"../uploads/".$upload_district."-".$_FILES['uploadedfile']['name'])){
            $goodtogo = false;
...
if ($goodtogo){
	...
	$target_path = $target_path . basename( $upload_district."-".$_FILES['uploadedfile']['name']);
