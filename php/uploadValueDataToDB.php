...
//erase known funny characters in upn
 foreach ($funnyChar as $key => $value) {
	...
	if ($_GET['ifproperty']=='1'){
		...
		$sdProperty->property_use='111';//$cellTemp['M'];
...
//Ascii Character 65=A to 90=Z
	...
	elseif ($_GET['ifproperty']=='0'){
		...
		$sdBusiness->da_no=$cellTemp['K'];
		$sdBusiness->business_certif=$cellTemp['L'];
		$sdBusiness->employees=$cellTemp['M'];
		$sdBusiness->business_name=$cellTemp['N'];
		$sdBusiness->year_establ=$cellTemp['O'];
		$sdBusiness->landmark1=$cellTemp['P'];
		$sdBusiness->landmark2=$cellTemp['Q'];
		$sdBusiness->owner=$cellTemp['R'];
		$sdBusiness->owneraddress=$cellTemp['S'];
		$sdBusiness->owner_tel=$cellTemp['T'];
		$sdBusiness->owner_email=$cellTemp['U'];
		$sdBusiness->business_class=$cellTemp['V'];
		$sdBusiness->comments='Uploaded by: uploadValueDataToDB.php - at: '.gmdate(DATE_RFC822).' - Comment: '.$cellTemp['G'];
		$sdBusiness->districtid=$_GET['districtid'];
...
//this is the part were the subupn is generated for the Property data
	...
	$resultdb = $sdProperty->find_dups_of_upn($_GET['districtid']);
// 	var_dump($resultdb);
	$a_temp= array();
	$a_t= array();
	$subupncounter=1;
...
//this is the part were the subupn is generated for the Business data
	$targetTable = $sdBusiness->tell_table_name();
	//get all duplicates from the table
	$resultdb = $sdBusiness->find_dups_of_upn($_GET['districtid']);
// 	var_dump($resultdb);
	$a_temp= array();
	$a_t= array();
	$subupncounter=1;
